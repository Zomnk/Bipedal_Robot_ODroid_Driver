...
public:
    ...
    bool transfer(const SPITxBuffer& tx_buffer, SPIRxBuffer& rx_buffer) {
        ...
        // 接收时: STM32 16位模式 + MSB first
        // 但实际测试显示数据是 LSB first (低字节先到)
        // 所以: rx_buf[i*2] = 低字节, rx_buf[i*2+1] = 高字节
        for (size_t i = 0; i < SPI_RX_WORDS; ++i) {
            uint8_t low  = rx_buf[i * 2];      // 先收到的是低字节
            uint8_t high = rx_buf[i * 2 + 1];  // 后收到的是高字节
