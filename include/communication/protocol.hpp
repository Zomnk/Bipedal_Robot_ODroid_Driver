 ...
 * @date 2026-02-01
 * 
 * 数据格式与STM32端ita_robot.cpp保持一致:
 * 
 * 控制数据 (TX to STM32): 40 x uint16
 *   左腿: [0-3]=髋关节, [4-7]=大腿, [8-11]=小腿, [12-15]=膝关节, [16-19]=踝关节
 *   右腿: [20-39] 同上
 *   每个关节: position, velocity, kp, kd
 * 
 * 反馈数据 (RX from STM32): 60 x uint16
 *   左腿: [0-3]=髋关节, [4-7]=大腿, [8-11]=小腿, [12-15]=膝关节
 *   右腿: [16-31] 同上
 *   IMU0: [32-41], IMU1: [42-51]
 *   保留: [52-59]
 * 
 * 编码规则: 
 *   float [-max, +max] <-> uint16 [0, 65535]
 *   0.0 -> 32768 (中点)
 */
...
/**
 * @brief 浮点数编码为uint16 (对称范围)
 * @param value 浮点值 [-max, +max]
 * @param max 最大值
...
/**
 * @brief uint16解码为浮点数 (对称范围)
 * @param value uint16 [0, 65535]
 * @param max 最大值
 * @return 浮点值 [-max, +max]
 */
...
/**
 * @brief 浮点数编码为uint16 (非对称范围)
 * @param value 浮点值 [min, max]
 * @param min 最小值
 * @param max 最大值
 * @return uint16 [0, 65535]
 */
...
public:
    ...
    static void decode_motor_fb(const uint16_t* buf, MotorFeedback& fb,
        ...
        fb.temperature = decode_float_range(buf[3], 0, 100);  // 温度 0-100°C
    }
    ...
    static void print_robot_fb(const RobotFeedback& fb) {
        ...
        LOG_DEBUG("Left leg:");
        LOG_DEBUG("  Hip:   pos=%.3f, vel=%.3f, torque=%.3f, temp=%.1f",
                  fb.left_leg.hip.position, fb.left_leg.hip.velocity,
                  fb.left_leg.hip.torque, fb.left_leg.hip.temperature);
        LOG_DEBUG("  Thigh: pos=%.3f, vel=%.3f, torque=%.3f, temp=%.1f",
                  fb.left_leg.thigh.position, fb.left_leg.thigh.velocity,
                  fb.left_leg.thigh.torque, fb.left_leg.thigh.temperature);
        // ... 其他关节类似
        
        LOG_DEBUG("IMU[0]: accel=[%.2f, %.2f, %.2f], gyro=[%.1f, %.1f, %.1f]",
