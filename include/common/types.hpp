/**
 * @file types.hpp
 * @brief 数据类型定义
 */

#ifndef ODROID_COMMON_TYPES_HPP
#define ODROID_COMMON_TYPES_HPP

#include <cstdint>
#include "constants.hpp"

namespace odroid {

//==============================================================================
// 电机相关类型
//==============================================================================

struct MotorCommand {
    float position = 0.0f;    // 目标位置 (rad)
    float velocity = 0.0f;    // 目标速度 (rad/s)
    float kp = 0.0f;          // 位置增益
    float kd = 0.0f;          // 速度增益
    float feedforward = 0.0f; // 前馈力矩 (Nm)
};

struct MotorFeedback {
    float position = 0.0f;    // 当前位置 (rad)
    float velocity = 0.0f;    // 当前速度 (rad/s)
    float torque = 0.0f;      // 当前力矩 (Nm)
    float temperature = 0.0f; // 温度 (C)
};

//==============================================================================
// 腿部相关类型
//==============================================================================

struct LegCommand {
    MotorCommand hip;         // 髋关节
    MotorCommand thigh;       // 大腿
    MotorCommand calf;        // 小腿
    MotorCommand knee;        // 膝关节
    MotorCommand ankle;       // 踝关节
};

struct LegFeedback {
    MotorFeedback hip;        // 髋关节
    MotorFeedback thigh;      // 大腿
    MotorFeedback calf;       // 小腿
    MotorFeedback knee;       // 膝关节
    MotorFeedback ankle;      // 踝关节
};

//==============================================================================
// IMU相关类型
//==============================================================================

struct IMUFeedback {
    float accel[3] = {0};     // 加速度 (m/s^2)
    float gyro[3] = {0};      // 角速度 (deg/s)
    float euler[3] = {0};     // 欧拉角 (deg)
    float temperature = 0.0f; // 温度 (C)
};

//==============================================================================
// 机器人整体类型
//==============================================================================

struct RobotCommand {
    LegCommand left_leg;      // 左腿
    LegCommand right_leg;     // 右腿
    uint64_t timestamp_us = 0;
};

struct RobotFeedback {
    LegFeedback left_leg;     // 左腿
    LegFeedback right_leg;    // 右腿
    IMUFeedback imu[2];       // 两个IMU
    uint64_t timestamp_us = 0;
};

//==============================================================================
// SPI缓冲区类型
//==============================================================================

struct SPITxBuffer {
    uint16_t data[SPI_TX_WORDS] = {0};
};

struct SPIRxBuffer {
    uint16_t data[SPI_RX_WORDS] = {0};
};

} // namespace odroid

#endif // ODROID_COMMON_TYPES_HPP
